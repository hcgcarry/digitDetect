...
if __name__=='__main__':
    ...
    batchNormalization_conv1=normalization(conv2d(x_image, W_conv1) + b_conv1,True)
    ...
    batchNormalization_conv2=normalization(conv2d(h_pool1, W_conv2) + b_conv2,True)
    ...
    for i in range(3001):
        batch_xs, batch_ys = mnist.train.next_batch(amount)
        rows=28
        cols=28
        ###img to affine
        img=np.reshape(batch_xs,(amount,28,28))
        pts1 = np.float32([[0,0],[cols-1,0],[0,rows-1]])
        pts2 = np.float32([[0,rows*0.3],[cols*0.8,0],[cols*0.1,rows*0.9]])
        M1 = cv2.getAffineTransform(pts1,pts2)
        pts1 = np.float32([[0,0],[cols-1,0],[0,rows-1]])
        pts2 = np.float32([[0,rows*0.15],[cols*0.9,0],[cols*0.05,rows*0.95]])
        M2 = cv2.getAffineTransform(pts1,pts2)
        pts1 = np.float32([[cols-1,rows-1],[cols-1,0],[0,0]])
        pts2 = np.float32([[cols*0.8,rows*0.9],[cols-1,rows*0.1],[cols*0.2,0]])
        M3 = cv2.getAffineTransform(pts1,pts2)
        pts1 = np.float32([[cols-1,rows-1],[cols-1,0],[0,0]])
        pts2 = np.float32([[cols*0.7,rows*0.8],[cols-1,rows*0.2],[cols*0.3,0]])
        M4 = cv2.getAffineTransform(pts1,pts2)
        #第三个参数：变换后的图像大小
        for index,item in enumerate(img):
            res1 = cv2.warpAffine(item,M1,(rows,cols))
            res1=np.reshape(res1,(28*28))
            affine_batch_xs.append(res1)
            affine_batch_ys.append(batch_ys[index])

    
            '''
            res1 = cv2.warpAffine(item,M2,(rows,cols))
            res1=np.reshape(res1,(28*28))
            affine_batch_xs.append(res1)
            affine_batch_ys.append(batch_ys[index])


            res1 = cv2.warpAffine(item,M3,(rows,cols))
            res1=np.reshape(res1,(28*28))
            affine_batch_xs.append(res1)
            affine_batch_ys.append(batch_ys[index])
            
            res1 = cv2.warpAffine(item,M4,(rows,cols))
            res1=np.reshape(res1,(28*28))
            affine_batch_xs.append(res1)
            affine_batch_ys.append(batch_ys[index])
            '''
        ...
        if i % 100==0:
            print('compelte one',i)
            affine_batch_xs=np.array(affine_batch_xs)
            affine_batch_ys=np.array(affine_batch_ys)
            sess.run(train_step, feed_dict={xs: affine_batch_xs, ys: affine_batch_ys, keep_prob: 0.5})
            affine_batch_xs=[]
            affine_batch_ys=[]
